...
def _build_action_plan_prompt(state: ShipyardState) -> str:
    ...
    lines = [
        ...
        "write_file, search_and_replace, append, prepend, anchor, rename_symbol, delete_file, copy_file, create_files, scaffold_files, list_files, read_file, read_many_files, search_files, run_command, verify_command, create_directory, move_file, rename_file, run_tests, inspect_imports.",
        "When using write_file on an existing file, the replacement must contain the COMPLETE file — all existing content plus your changes. Do not drop content the instruction didn't ask you to remove.",
        "For new files: use write_file (single) or scaffold_files (multiple).",
        "",
