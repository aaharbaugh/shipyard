...
class ActionPlannerTests(unittest.TestCase):
    ...
    def test_openai_action_plan_accepts_write_file_on_existing_file_without_full_rewrite(self) -> None:
        """full_file_rewrite is advisory metadata — write_file should succeed without it."""
        action_response = Mock()
        action_response.json.return_value = {
        ...
        self.assertFalse(
            any(
                "full_file_rewrite" in error
                for error in result["actions"][0].get("validation_errors", [])
            )
        )
